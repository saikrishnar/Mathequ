...
#subscript
  if mtag == 'msub':
    ...
    subNode =et.SubElement(snode,'PITCH',BASE = '-25%')
    mathparse(element[1],subNode,exp)
    if len(subNode) > 0:
...
#subscript-superscript pairs
  if mtag == 'msubsup':
    ...
    et.SubElement(snode,'EMPH').text = 'subscript'
    ssSub = et.SubElement(snode,'PITCH',BASE='-25%')
    ...
    exp = []
    et.SubElement(snode,'EMPH').text = 'superscript'
    ssSup = et.SubElement(snode,'PITCH',BASE = '+25%')
...
# square root
  if mtag == 'msqrt':
    ...
    exp = []
    et.SubElement(snode,'BREAK',LEVEL='Medium')
    sqrtNode = et.SubElement('RATE',SPEED='+55%')
    for c in element:
